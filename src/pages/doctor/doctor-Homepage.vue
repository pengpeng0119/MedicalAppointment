        ...
        <scroll-view scroll-x="true" class="scroll-view_H" enable-flex="true">
          ...
          >
            ...
            <view
              ...
              "
            >
              {{ item_a.time[0].nu_source <= 0 ? '无' : '预约' }}
            </view>
            <!-- 下午 -->
            <view
              ...
              "
            >
              {{ item_a.time[1].nu_source <= 0 ? '无' : '预约' }}
            </view>
          </view>
        </scroll-view>
      </view>
...
import SkeLeton from '@/Skeleton/SK-doctor-home.vue'
let _id = ref('')
...
onLoad(async (event) => {
  ...
  _id.value = id
  const res = (await RequestApi.DoctorHome({ _id: _id.value })) as {
