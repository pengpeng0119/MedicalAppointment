<template>
  ...
  <view class="hpv-list" v-for="(item, index) in hpv_list" :key="index">
    <view>
      ...
      <view class="hpv-age">
        <text v-for="(item_a, index_a) in item.describe" :key="index">{{
          item_a
        }}</text>
      </view>
      <view class="hpv-price">{{ item.price[0] }}元-{{ item.price[1] }}元</view>
    </view>
    <view
      class="hpv-reserve"
...
import skeLeton from '@/Skeleton/SK-hpvlist.vue'
// 筛选的导航
let checked = ref(0)
...
function toggle(_id: string, index: number) {
  let res = hpv_all.value.filter((item) => item.hpv_id == _id)
  hpv_list.value = res
  checked.value = index
  // console.log(res)
  if (_id == '26da8e4962dc565503df9629704f1700') {
...
function toReserve(
  ...
  let obj = JSON.stringify({ id, name, price, describe })
  uni.navigateTo({
    url: '/pages/hpv-vaccine/buy-vaccine?value=' + obj
  })
}
</script>
