<template>
  <view class="reg-view" v-for="(item, index) in order" :key="index">
    ...
    <view class="reg-order">
      <text>就诊时间</text>
      <text>日期{{ item.tre_time }} 时间{{ item.the_time }}</text>
    </view>
    <view class="reg-order">
      <text>科室</text>
    ...
    <view class="reg-cancel">
      <text
        :class="item.cancel === true ? '' : 'prevent_style'"
        @click="Cancel(item._id, index)"
        >{{ item.cancel === true ? '取消预约' : '已取消预约' }}</text
      >
    </view>
  </view>
  <!-- 提示组件 -->
...
import type { Registratorder } from '@/public/decl-type'
// 提示组件
...
let show = ref(false)
onShow(async () => {
  const res = (await RequestApi.UserreGistrat()) as {
    data: { data: Registratorder[] }
  }
