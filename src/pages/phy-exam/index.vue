<template>
    ...
    <view class="con-view" v-show="conview">
        <block v-if="phy_term.length > 0"><text v-for="(item, index) in phy_term[0].filter_val" :key="index"
                @click="selectType(item)">{{ item
                }}</text></block>
    </view>
    <!-- 列表 -->
...
function queryCombo(index: number, value: string[]) {
    if (index === 0) {
        conview.value = conview.value == true ? false : true
...
// 到体检详情页
function phyDetail(_id: string, title: string) {
    uni.navigateTo({
        url: "/pages/phy-exam/Details?id=" + _id + '&title=' + title
    })
}
</script>
