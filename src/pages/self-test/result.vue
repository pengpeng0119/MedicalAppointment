<template>
  <view class="back-color">
    ...
    <view class="result-desc" v-for="(item, index) in test_res" :key="index">
      ...
      <block v-if="item.recommend.length > 0">
        ...
        <view
          class="recommend"
          v-for="(item_b, index_b) in item.recommend"
          :key="index_b"
        >
          <label>
            <text class="bold">{{ item_b.dep_name }}</text>
            <text class="Hosp">{{ item_b.hospital }}</text>
          </label>
  ...
  <ske-letons v-if="skeLeton"></ske-letons>
</template>
...
import skeLetons from '@/Skeleton/SK-self-test-result.vue'
...
onShow(async () => {
  if (type_id.value == '001') {
    //抑郁症
    var res = (await RequestApi.DepressiSon({ value: topic_id_v.value })) as Res
