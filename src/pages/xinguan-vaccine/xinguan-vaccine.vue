<template>
    ...
    <view class="xinguan-view">
        <view class="xinguan-flex">
            <text>姓名</text>
            <input placeholder-class="input-style" placeholder="请输入姓名" v-model="penging_data.name" />
        </view>
        <view class="xinguan-flex">
            <text>身份证</text>
            <input placeholder-class="input-style" placeholder="请输入身份证" v-model="penging_data.id_card" />
        </view>
        <view class="xinguan-flex">
            <text>手机号</text>
            <input placeholder-class="input-style" placeholder="请输入手机号" v-model="penging_data.phone" />
        </view>
        <view class="xinguan-flex">
            <text>现居地址</text>
            <picker mode="region" class="flex-left" @change="changeRegion">
                <view>
                    <text>{{ penging_data.address === '' ? '请输入现居地址' : penging_data.address }}</text>
                    <image sre="../../static/other/gengduo.svg"></image>
                </view>
            </picker>
        </view>
        <view class="xinguan-flex">
            <text>现居详细地址</text>
            <input placeholder-class="input-style" placeholder="请输入现居详细地址" v-model="penging_data.de_address" />
        </view>
        <view class="xinguan-flex">
            <text>疫苗人群分类</text>
...
onMounted(async () => {
    ...
    console.log(timeData.data.Hospital)
})
...
// 选择时段
let timeIndex = ref('1')
// ChooseTime(index+'-'+index_a,item.period,item_a.start_time,item_a.end_time,item_a.when)
